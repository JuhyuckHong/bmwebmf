...
function SingleSite({ site, setSite }) {
  ...
  useEffect(() => {
    ...
    const getAndSetRecentImage = async () => {
      try {
        ...
        const response = await API.getRecent({ Authorization: cookie.load("BM") }, site);  // Replace {} with your headers
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
      ...
      <h2>{site}</h2>
      {imageUrl && <img src={imageUrl} alt="Recent" style={{ maxWidth: '100%', maxHeight: '80vh', objectFit: 'contain' }} />}
